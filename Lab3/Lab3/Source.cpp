...
int main()
{
	...
	/////// Structuri de date ///////////
	//using data structure, we can create our own data types
	//a new custom datatype in order to create the app for our solution

	Student s;

	//when we are using the pointers for arrays,we can modify the initial memory area
	s.id = 12;
...
int params_example_1(int param1, int* param2, char* param3)
{
	param1 += 1; //p1 will not be incremented because its not value based
...
int params_example_2(int param1, int* param2, char* param3)
{
	...
	param2[0] = 102; //we are modifying the value because its an array
...
void params_ex(int p1, int p2, int p3)
{
	...
	//1.modify the values from main functions
	p1 += 1;
	p2 += 1;
	p3 += 1;
...
void ref_param(Student& s) //whats different compared to the other functions? - by reference"&"
//we dont use pointers, we dont use deref
{
	s.nume[0] = '@';
	s.data_nasterii[0] = -1;
}
