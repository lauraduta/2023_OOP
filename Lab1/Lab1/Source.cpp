...
//when we want to use a function we have to declare it in main before we call it

void print_data_types_sizes();

//{}
//it's mot wrong to write the function like this^, but we prefer it like this so we can see the main function first (it's easier to see)
...
int main()
{
	...
	// Data tyes size
	print_data_types_sizes(); //when the arrow is here, it shows that it's the next line about to be executed
...
void scope_operator_example() //what is executed in this function -> 2 variables with the same name: local:1234, global: another value
//how can we acces variables with the same name, local and global - with the scope operator
{
	int var = 1234;
...
void pointers_example_1()
{
	...
	int a = 3;
	int b = 5; //the value is not set to the variable from watch 1
	int var = 5478;
	int* ptr_a = nullptr; // the diff between nullptr and null variables -> 
	int* ptr_b = nullptr;
	int* ptr_var = nullptr;
