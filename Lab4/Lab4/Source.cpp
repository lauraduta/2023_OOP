	...
	*/
	...
	void set_note(int* _note, int _numar_materii)
	{
		...
		if (this->note != nullptr) {
			delete[] this->note;		//we only deallocate space for one int
		}
	...
	int* get_note() {
		...
		return copy;			//memory must be deallocated outside the function
	}
